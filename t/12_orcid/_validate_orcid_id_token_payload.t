...
use warnings;
use utf8;
use Test::More skip_all => 'not sure we need this';
...
_VALIDATE_PAYLOAD: {
    ...
    my $result = 
        OIDC::Lite::Demo::Client::Web::C::ORCID->_validate_orcid_id_token_payload( $payload, $config );
    ok( $result, q{no iss} );
    ok( !$result->{status}, q{status is invalid} );
    is( $result->{message}, q{iss does not exist}, q{iss does not exist} );

    $payload = {
       iss => q{this.is.not.orcid}, 
    };
    $result = 
        OIDC::Lite::Demo::Client::Web::C::ORCID->_validate_orcid_id_token_payload( $payload, $config );
    ok( $result, q{invalid iss} );
    ok( !$result->{status}, q{status is invalid} );
    is( $result->{message}, q{iss is not ORCID}, q{iss is not ORCID} );
    is( $result->{iss}, $payload->{iss}, q{iss is set} );

    $payload = {
       iss => q{accounts.orcid.com}, 
    };
    $result = 
        OIDC::Lite::Demo::Client::Web::C::ORCID->_validate_orcid_id_token_payload( $payload, $config );
    ok( $result, q{no iat} );

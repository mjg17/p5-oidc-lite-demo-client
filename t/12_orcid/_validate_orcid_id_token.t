use strict;
use warnings;
use utf8;
use Test::More;
use Test::MockObject;
use OIDC::Lite::Demo::Client::Web::C::ORCID;
use JSON qw/encode_json/;

my $content = {
    "5404196e2d38cca061bc51a55a04bc24c1612184"=> "-----BEGIN CERTIFICATE-----

TuDVYQTK/BL2ixH1+sHczdBhuJeu
-----END CERTIFICATE-----",
    "07f6fe44e331d161e238c06ff722bca2c79b82b2"=> "-----BEGIN CERTIFICATE-----

8sA1wxL8d8BuCtjPFCmXCmlP1i/B
-----END CERTIFICATE-----",
    "invalid_key" => "invalid",
};
my $content_is_not_json = q{not json string};

_GET_ORCID_PUB_KEY: {
    Test::MockObject->fake_module(
        'LWP::UserAgent',
        'new' => sub{bless {}, shift},
        'request' => sub {
            my ($class, $req) = @_;
            return HTTP::Response->new(400, '', undef, undef);
        },
    );
    my $pub = 
        OIDC::Lite::Demo::Client::Web::C::ORCID->_get_orcid_pub_key("dummy");
    ok( !$pub, q{HTTP status code is not ok});

    Test::MockObject->fake_module(
        'LWP::UserAgent',
        'new' => sub{bless {}, shift},
        'request' => sub {
            my ($class, $req) = @_;
            return HTTP::Response->new(200, '', undef, $content_is_not_json);
        },
    );
    $pub = OIDC::Lite::Demo::Client::Web::C::ORCID->_get_orcid_pub_key("dummy");
    ok( !$pub, q{content is not json});

    Test::MockObject->fake_module(
        'LWP::UserAgent',
        'new' => sub{bless {}, shift},
        'request' => sub {
            my ($class, $req) = @_;
            my $res = HTTP::Response->new(200, '', undef, encode_json($content));
        },
    );
    $pub = OIDC::Lite::Demo::Client::Web::C::ORCID->_get_orcid_pub_key("invalid_key");
    ok( !$pub, q{pubkey is not PEM});

    $pub = OIDC::Lite::Demo::Client::Web::C::ORCID->_get_orcid_pub_key("5404196e2d38cca061bc51a55a04bc24c1612184");
    ok( $pub, q{pubkey returned});
    use Data::Dumper;
    like( $pub, qr/-----BEGIN PUBLIC KEY-----/, q{-----BEGIN PUBLIC KEY-----});
    like( $pub, qr/-----END PUBLIC KEY-----/, q{-----END PUBLIC KEY-----});
};

_VALIDATE_ORCID_ID_TOKEN: {
    Test::MockObject->fake_module(
        'LWP::UserAgent',
        'new' => sub{bless {}, shift},
        'request' => sub {
            my ($class, $req) = @_;
            my $res = HTTP::Response->new(200, '', undef, encode_json($content));
        },
    );

    # not JWS
    my $id_token = q{This is not JWS string};
    my $result = OIDC::Lite::Demo::Client::Web::C::ORCID->_validate_orcid_id_token( $id_token );
    ok( $result, q{result is returned});
    is( $result->{id_token_string}, $id_token, q{id_token_string is set});
    ok( !$result->{signature_status}, q{signature status is invalid});
    ok( !$result->{encoded_header}, q{encoded_header is not set});

    # no alg
    $id_token = q{eyJmb28iOiJiYXIifQ.eyJmb28iOiJiYXIifQ.invalid_signature};
    $result = OIDC::Lite::Demo::Client::Web::C::ORCID->_validate_orcid_id_token( $id_token );
    ok( $result, q{result is returned});
    ok( !$result->{signature_status}, q{signature status is invalid});
    is( $result->{encoded_header}, q{eyJmb28iOiJiYXIifQ}, q{encoded_header is set});
    is( $result->{encoded_payload}, q{eyJmb28iOiJiYXIifQ}, q{encoded_payload is set});
    is( $result->{encoded_signature}, q{invalid_signature}, q{encoded_signature is set});
    is( $result->{signing_input}, q{eyJmb28iOiJiYXIifQ.eyJmb28iOiJiYXIifQ}, q{signing_input is set});
    ok( !$result->{header_content}, q{header is not set});

    # alg is invalid
    $id_token = q{eyJhbGciOiJIUzI1NiJ9.eyJmb28iOiJiYXIifQ.invalid_signature};
    $result = OIDC::Lite::Demo::Client::Web::C::ORCID->_validate_orcid_id_token( $id_token );
    ok( $result, q{result is returned});
    ok( !$result->{signature_status}, q{signature status is invalid});
    ok( !$result->{header_content}, q{header is not set});

    # no kid
    $id_token = q{eyJhbGciOiJSUzI1NiJ9.eyJmb28iOiJiYXIifQ.invalid_signature};
    $result = OIDC::Lite::Demo::Client::Web::C::ORCID->_validate_orcid_id_token( $id_token );
    ok( $result, q{result is returned});
    ok( !$result->{signature_status}, q{signature status is invalid});
    ok( !$result->{header_content}, q{header is not set});

    # invalid kid
    $id_token = q{eyJhbGciOiJSUzI1NiIsImtpZCI6ImludmFsaWQifQ.eyJmb28iOiJiYXIifQ.invalid_signature};
    $result = OIDC::Lite::Demo::Client::Web::C::ORCID->_validate_orcid_id_token( $id_token );
    ok( $result, q{result is returned});
    ok( !$result->{signature_status}, q{signature status is invalid});
    ok( $result->{header_content}, q{header is set});
    ok( !$result->{pubkey}, q{pubkey is not set});

    # invalid signature
    $id_token = q{eyJhbGciOiJSUzI1NiIsImtpZCI6IjU0MDQxOTZlMmQzOGNjYTA2MWJjNTFhNTVhMDRiYzI0YzE2MTIxODQifQ.eyJmb28iOiJiYXIifQ.invalid_signature};
    $result = OIDC::Lite::Demo::Client::Web::C::ORCID->_validate_orcid_id_token( $id_token );
    ok( $result, q{result is returned});
    ok( !$result->{signature_status}, q{signature status is invalid});
    ok( $result->{header_content}, q{header is set});
    ok( $result->{pubkey}, q{pubkey is set});

    # valid signature
    $id_token = q{eyJhbGciOiJSUzI1NiIsImtpZCI6IjA3ZjZmZTQ0ZTMzMWQxNjFlMjM4YzA2ZmY3MjJiY2EyYzc5YjgyYjIifQ..VagTki3Ap29EDgrC38cmGtZeoZdPJjPH4f_475Nd2FVa84HQDkSrO4-3r-2sxco_Z8DWi_i-AlHLHC01JTmOg69inwAWQ2usPV9gkcNLt5PMKKc3TkfB5WM6R5lNz-H6NHVldVkV1GHKTBFoFs4lwyE83ko41EHdHKxmgj9L5cM};
    $result = OIDC::Lite::Demo::Client::Web::C::ORCID->_validate_orcid_id_token( $id_token );
    ok( $result, q{result is returned});
    ok( $result->{signature_status}, q{signature status is valid});
    ok( $result->{payload_content}, q{payload});
    ok( $result->{payload}, q{payload});
};

done_testing;

...
_VALIDATE_PAYLOAD: {
    ...
    my $result = 
        OIDC::Lite::Demo::Client::Web::C::Otter->_validate_otter_id_token_payload( $payload, $config );
    ok( $result, q{no iss} );
    ok( !$result->{status}, q{status is invalid} );
    is( $result->{message}, q{iss does not exist}, q{iss does not exist} );

    $payload = {
       iss => q{this.is.not.otter}, 
    };
    $result = 
        OIDC::Lite::Demo::Client::Web::C::Otter->_validate_otter_id_token_payload( $payload, $config );
    ok( $result, q{invalid iss} );
    ok( !$result->{status}, q{status is invalid} );
    is( $result->{message}, q{iss is not Otter}, q{iss is not Otter} );
    is( $result->{iss}, $payload->{iss}, q{iss is set} );

    $payload = {
       iss => q{accounts.otter.com}, 
    };
    $result = 
        OIDC::Lite::Demo::Client::Web::C::Otter->_validate_otter_id_token_payload( $payload, $config );
    ok( $result, q{no iat} );
